const sampler_t nb_sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP | CLK_FILTER_NEAREST;
...
__kernel
void positivity (__read_only image2d_t in,
                 __write_only image2d_t out)
{
    int2 coord;
    ...
    float val = read_imagef(in, nb_sampler, coord).s0;

    write_imagef(out, coord, val > 0 ? val : 0);
}